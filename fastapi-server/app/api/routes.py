...
async def orchestrate(req: OrchestrationRequest):
    ...
    """
    try:
        # Simple keyword-based routing for demo (without API keys)
        user_prompt_lower = req.user_prompt.lower()
        
        # Route to the appropriate service based on keywords
        if any(word in user_prompt_lower for word in ["shelter", "homeless", "sleep", "bed"]):
            return await handle_shelter_request(req.latitude, req.longitude)
        elif any(word in user_prompt_lower for word in ["medical", "doctor", "hospital", "health", "sick"]):
            return await handle_medical_center_request(req.latitude, req.longitude)
        elif any(word in user_prompt_lower for word in ["bathroom", "restroom", "toilet", "washroom"]):
            return await handle_restroom_request(req.latitude, req.longitude)
        elif any(word in user_prompt_lower for word in ["pharmacy", "medicine", "drug", "prescription"]):
            return await handle_pharmacy_request(req.latitude, req.longitude)
        else:
            # Default to shelter if no specific keywords found
            return await handle_shelter_request(req.latitude, req.longitude)
