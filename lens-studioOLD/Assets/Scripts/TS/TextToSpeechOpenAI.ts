...
export class TextToSpeechOpenAI extends BaseScriptComponent {
	...
	@input('string')
	apiKey: string = 'your-openai-api-key-here'
	...
	onAwake() {
		try {
			if (!this.audioComponent) {
				print('Audio Component is missing.')
				return
			}

			if (!this.apiKey || this.apiKey === 'your-openai-api-key-here') {
				print('API key is missing or not configured. Set it in the Inspector.')
				return
			}
			...
			print('Error in TextToSpeechOpenAI onAwake: ' + error)
		}
	}
