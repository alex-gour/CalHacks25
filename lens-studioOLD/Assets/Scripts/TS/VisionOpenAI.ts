...
export class VisionOpenAI extends BaseScriptComponent {
	...
	async handleTriggerEnd(eventData) {
		...
		try {
			...
			// Get texture from assigned image component
			if (this.image && this.image.mainPass && this.image.mainPass.baseTex) {
				...
				print('ERROR: Image component not properly assigned or has no texture.')
				print('IMPORTANT: In Lens Studio Inspector, make sure the "Image" field is connected to a camera texture, NOT a static preview image!')
				if (this.LLM_analyse) {
					this.LLM_analyse.text = this.makeTextWrappable('Error: Camera not set up correctly. Check console for details.')
				}
